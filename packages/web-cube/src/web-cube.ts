...
export class WebCube extends HTMLElement {
  ...
  #cubeGroups: { [key: string]: HTMLDivElement[][] } | null = null;
  ...
  #createPointerEvents() {
    this.#viewportPointerEvent = (ev) => {
      ...
      const $closestFace = $target.closest(
        ".face.sticker",
      ) as HTMLElement | null;
      ...
      if (currentFace === null) {
          ...
          if (!axis) {
            ...
            if (xDistance > 10 || yDistance > 10) {
              if (xDistance > yDistance) {
                axis = "y";
              } else if (currentFace === Face.Front) {
                axis = "x";
              } else {
                axis = "z";
              }
        ...
        function handlePointerMove(ev: PointerEvent) {
          ...
          const rotatedX =
            Math.cos(Math.PI / 6) * xDistance -
            Math.sin(Math.PI / 6) * yDistance;
          const rotatedY =
            Math.sin(Math.PI / 3) * xDistance +
  ...
  async #rotateAxisLayer({
    ...
    if (angle !== 360 && angle !== 0) {
      const minimunAngle = angle === 270 ? 90 : angle;
      const realBackwards = angle === 270 ? !backwards : backwards;
      const posibleRotation = ROTATIONS[axis][minimunAngle];
      if (minimunAngle === 90) {
  ...
  async #rotateAxisCube({
    ...
    if (angle !== 360 && angle !== 0) {
      const minimunAngle = angle === 270 ? 90 : angle;
      const realBackwards = angle === 270 ? !backwards : backwards;
      const posibleRotation = ROTATIONS.cube[axis][minimunAngle];
      if (minimunAngle === 90) {
        rotation = realBackwards
          ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (posibleRotation as any).backwards
